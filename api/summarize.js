export default async function handler(req,res){
  ...
  if(!key)return res.status(503).json({error:"AI summarization is not configured. Add OPENAI_API_KEY to the Vercel project environment variables."});
  try{
    const body=req.body||{};
    const filename=String(body.filename||"document.pdf").slice(0,200);
    const fileData=String(body.fileData||"");
    const company=String(body.company||"").trim();
    if(!fileData.startsWith("data:application/pdf;base64,"))return res.status(400).json({error:"Please upload a valid PDF."});
    const raw=fileData.split(",")[1]||"";
    if(!raw)return res.status(400).json({error:"Empty PDF."});
    if(raw.length>50*1024*1024*1.4)return res.status(413).json({error:"PDF is too large. Please use a PDF under 50 MB."});
    ...
    const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Authorization":"Bearer "+key,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-5.6-luna",input:[{role:"user",content:[{type:"input_file",filename,file_data:fileData,detail:"high"},{type:"input_text",text:prompt}]}],max_output_tokens:8000})});
    ...
    return res.status(200).json({filename,company,summary:text,model:"gpt-5.6-luna"});
