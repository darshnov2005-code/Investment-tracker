...
function table(h){
  ...
  return "<div class=\"tablewrap\"><table class=\"table\"><thead><tr><th>Asset</th><th>Qty</th><th>Avg</th><th>Current</th><th>Value</th><th>Unrealised P/L</th><th>Actions</th></tr></thead><tbody>"+
    h.map(x=>"<tr><td><div class=\"asset\">"+esc(x.name)+"</div><div class=\"sub\">"+esc(x.symbol)+" · "+esc(x.exchange)+"</div></td><td>"+num(x.qty)+"</td><td>"+money(x.avg)+"</td><td>"+money(x.current)+"</td><td>"+money(x.value)+"</td><td class=\""+(x.pnl>=0?"green":"red")+"\">"+money(x.pnl)+"<div class=\"sub\">"+pct(x.cost?x.pnl/x.cost*100:0)+"</div></td><td><div style=\"display:flex;gap:5px;flex-wrap:wrap\"><button class=\"btn\" data-buy=\""+esc(x.symbol)+"\">Buy</button><button class=\"btn\" data-sell=\""+esc(x.symbol)+"\">Sell</button><button class=\"btn\" data-edit=\""+esc(x.symbol)+"\">Edit</button></div></td></tr>").join("")+
    "</tbody></table></div>";
}
function insights(t){
